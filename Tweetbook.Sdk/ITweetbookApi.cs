...
namespace Tweetbook.Sdk
{
    ...
    public interface ITweetbookApi
    {
        ...
        Task<ApiResponse<PostResponse>> GetAsync(string postId);

        [Post("/api/v1/posts")]
        Task<ApiResponse<PostResponse>> CreateAsync([Body] CreatePostRequest createPostRequest);

        [Put("/api/v1/posts/{postId}")]
        Task<ApiResponse<PostResponse>> UpdateAsync(string postId, [Body] UpdatePostRequest updatePostRequest);

        [Delete("/api/v1/posts/{postId}")]
        Task<ApiResponse<string>> DeleteAsync(string postId);
    }
}
